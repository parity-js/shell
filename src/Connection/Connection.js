...
class Connection extends Component {
      ...
      <div>
        ...
        <div className={styles.modal}>
          <GradientBg className={styles.body}>
            ...
            {needsToken ? this.renderSigner() : this.renderPing()}
          </GradientBg>
        </div>
      </div>
    );
        ...
        <div className={styles.info}>
          ...
          <div className={styles.form}>
            <Input
              ...
              disabled={loading}
              error={
                validToken || (!token || !token.length) ? null : (
                  <FormattedMessage
  ...
  validateToken = _token => {
    ...
    const validToken = /^[a-zA-Z0-9]{4}(-)?[a-zA-Z0-9]{4}(-)?[a-zA-Z0-9]{4}(-)?[a-zA-Z0-9]{4}$/.test(
      token
    );
  ...
  onChangeToken = (event, _token) => {
    const { token, validToken } = this.validateToken(
  ...
  setToken = () => {
    ...
    return api.updateToken(token, 0).then(isValid => {
      this.setState({
